...
module.exports = (props) => {
    props = {retries: 3, initialBackoff: 1000, rateLimit: 0, failThreshold: 10, successThreshold: 3, breakerTimeout: 10000, backoffFunction : (delay,initial,attempt) => delay+initial, retryBreaker: false, circuitBreaker: true, ...props}
    ...
    return (asyncErrorHandler, asyncFallback, asyncFunction) => async (...params) => {
        ...
        while(tries-->-1){
            ...
            if(props.circuitBreaker && breakerState===0) {
                ...
                if(props.retryBreaker) await slep(delay = props.backoffFunction(delay,props.initialBackoff,(props.retries-tries)));
                else return asyncFallback ? await asyncFallback(...params) : null;
            }else{
                if(props.rate>0){
                    const rate = lastCall ? Date.now() - lastCall : props.rateLimit;
                    lastCall = Date.now() + (rate < props.rate ? props.rateLimit - rate : 0);
                    if(rate < props.rateLimit ) await slep(props.rateLimit - rate);
                }
                ...
                if(props.backoffFunction) await slep(delay = props.backoffFunction(delay,props.initialBackoff,(props.retries-tries)));
            }
        }
