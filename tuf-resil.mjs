    ...
    return (asyncErrorHandler, asyncFallback, asyncFunction) => async (...params) => {
        ...
        while(tries-->-1){
            ...
            if(props.circuitBreaker && breakerState===0) {
                if(asyncErrorHandler) await asyncErrorHandler({source: 'tuf', error: new Error('Breaker Closed'), circuitBreaker: props.circuitBreaker, breakerState, failures, successes, tries: props.retries-tries, props});
                ...
                if(asyncErrorHandler) await asyncErrorHandler({source: 'fn', error: err, circuitBreaker: props.circuitBreaker, breakerState, failures, successes, tries: props.retries-tries, props});
                if(props.backoffFunction) await slep(delay = props.backoffFunction(delay,props.backoff,(props.retries-tries)));
            }
        }
        if(asyncErrorHandler) await asyncErrorHandler({source: 'tuf', error: new Error('Out of retries'), circuitBreaker: props.circuitBreaker, breakerState, failures, successes, tries: props.retries-tries, props});
